...
local function EnableNUI()
    ...
    SendNUIMessage({response = 'clickTarget'})
    SetCursorLocation(0.5, 0.5)
    SetNuiFocusKeepInput(true) -- Garde le focus sur l'interface NUI
    hasFocus = true
end
			...
			if entityType > 0 then
				...
				elseif entityType == 2 then
					...
					if data and next(data) and closestBone then
						SetupOptions(data, entity, distance, false)
						print(data, entity, distance, closestBoneName)
...
RegisterNUICallback('selectTarget', function(msgData, cb)
    local option = tonumber(msgData.id) or msgData.id  -- Utilise l'ID de msgData
    cb()
    ...
    CreateThread(function()
        ...
        elseif data.event then
            local event, _type = data.event, data.type
